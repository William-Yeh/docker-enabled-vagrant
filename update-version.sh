...
for f in "${FILES[@]}" ; do
    ...
    sed -i -e 's/COMPOSE_VERSION=.*/COMPOSE_VERSION=1.7.0/g'   $f
    ...
    sed -i -e 's/ETCD_VERSION=.*/ETCD_VERSION=v2.3.1/g'               $f
    sed -i -e 's/KUBERNETES_VERSION=.*/KUBERNETES_VERSION=v1.2.2/g'   $f
