...
for f in "${FILES[@]}" ; do
    ...
    sed -i -e 's/MACHINE_VERSION=.*/MACHINE_VERSION=v0.8.0/g'  $f
    ...
    sed -i -e 's/CADVISOR_VERSION=.*/CADVISOR_VERSION=0.23.4/g'    $f

    sed -i -e 's/ETCD_VERSION=.*/ETCD_VERSION=v3.0.4/g'               $f
    sed -i -e 's/KUBERNETES_VERSION=.*/KUBERNETES_VERSION=v1.3.3/g'   $f
